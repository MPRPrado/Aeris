...
function Graficos02() {
  ...
  useEffect(() => {
    ...
    const buscarRelatorio = async () => {
      try {
        const response = await axios.get('http://localhost:8000/mq7/relatorio/');
        if (response.data) {
    ...
    const buscarDados = async () => {
      try {
        ...
        const valoresAltos = ultimos70.filter(item => item.valor > 300);
        
       
        const agora = Math.floor(Date.now() / 60000); 
        const ultimoAlerta = localStorage.getItem('ultimoAlertaMQ7');
        
        if (valoresAltos.length > 0 && ultimoAlerta !== agora.toString()) {
          setShowGasAlert(true);
          localStorage.setItem('ultimoAlertaMQ7', agora.toString());
          setTimeout(() => {
...
📡 {espSelecionado.nome}
          ...
          <div className="frases-abaixo-grafico">
            ...
            <p><strong>Média:</strong> entre 1001 e 8000 ppm (nível típico de ambientes urbanos ou internos)</p>
            <p><strong>Alta:</strong> acima de 8001 ppm (pode indicar ventilação insuficiente ou acúmulo de emissões)</p>
          </div>
        </div>
        <div className="caixa-lateral-menor">
          <div>
            ...
            <button 
              onClick={() => window.open('http://localhost:8000/mq7/download-pdf/', '_blank')}
              style={{
                marginTop: '15px',
                padding: '10px 20px',
